...
7. исходящее событие сначала сохраняется в outbox
8. успешная отправка помечает outbox-запись как `SENT`
9. ошибка отправки помечает outbox-запись как `FAILED`
10. `flush_outbox()` повторно отправляет неуспешные записи
