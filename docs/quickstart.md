...
При `publish(event)` библиотека:

1. сохраняет событие в outbox-таблицу
2. пытается отправить его в RabbitMQ
3. при успехе помечает запись как `SENT`
4. при ошибке помечает запись как `FAILED`
...
4. `publish(event)` сначала пишет событие в outbox, а потом отправляет в RabbitMQ
5. если RabbitMQ недоступен, исходящее событие не теряется
6. для повторной отправки можно использовать `flush_outbox()`
